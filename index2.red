Red [
    ...
    .Read: function [
        ...
        source [file! url! string! unset!] /part {Partial read a given number of units (source relative)} 
        length [number!] /seek "Read from a specific position (source relative)" 
        index [number!] /binary "Preserves contents exactly" 
        ...
        bin-to-string: function [bin [binary!]] [text: make string! length? bin 
            foreach byte bin [append text to char! byte] text
        ] 
        source?: true 
    ...
    .html-gen: function [
        ...
        use ["alert messages you can customize"] [
            ...
            emit-links: function [links-collection] [foreach [title url] links-collection [
                either not block? title [
                    either ((not url? title) and (not file? title)) [url-block: reduce [title url] emit-link url-block] [emit-link title 
                        emit-link url
                    ]
                ] [
                    emit-link title 
